...
function getProjectTheme(project) {
    ...
    if (key.includes('api') || key.includes('integration')) return 'theme-saas';
...
function getProjectThemeLabel(project) {
    const key = (project.domain || project.category || '').toLowerCase();
    if (key.includes('stream') || key.includes('entertainment')) return 'Streaming & Entertainment';
    if (key.includes('e-commerce') || key.includes('ecommerce') || key.includes('retail')) return 'Retail Experience';
    if (key.includes('blog') || key.includes('news') || key.includes('wiki')) return 'Content & Editorial';
    if (key.includes('corporate') || key.includes('business') || key.includes('agency')) return 'Corporate Presence';
    if (key.includes('saas') || key.includes('platform') || key.includes('analytics')) return 'SaaS Experience';
    if (key.includes('api') || key.includes('integration')) return 'API & Integration';
    if (key.includes('health') || key.includes('fitness') || key.includes('medical')) return 'Health & Wellness';
    if (key.includes('education') || key.includes('learning') || key.includes('course')) return 'Learning Platform';
    if (key.includes('social') || key.includes('community') || key.includes('forum')) return 'Community Hub';
...
function renderProjects() {
    ...
    el.innerHTML = pageProjects.map(item => {
        const detailLink = item.url || item.detailsUrl || `project-detail.html?id=${encodeURIComponent(item.id)}`;
        const statusClass = item.status === 'Live' || item.status === 'Active' ? 'live' : item.status === 'Demo' ? 'demo' : 'other';
        const themeClass = getProjectTheme(item);
        const statusIcon = item.status === 'Live' || item.status === 'Active' ? '🟢' : item.status === 'Demo' ? '🔗' : '📋';
        const themeLabel = getProjectThemeLabel(item);
        return `
        <a href="${detailLink}" aria-label="View details for ${item.name}">
            <article class="module-card reveal visible ${statusClass} ${themeClass}">
                ...
                <p class="module-blurb">Responsive frontend paired with backend APIs for every project.</p>
                <span class="module-badge">${item.capabilities.join(' · ')}</span>
                <span class="module-tag">${themeLabel}</span>
...
// ─── CONTACT FORM ───
// Submit contact messages through a serverless endpoint when available,
// with a mailto fallback to keep the experience responsive on static hosting.
document.getElementById('contact-form').addEventListener('submit', async e => {
    ...
    fb.textContent = 'Sending your request…';
    ...
    try {
        ...
        if (response.ok && result.success) {
            fb.textContent = '✓ Message received. Thank you — we will follow up shortly.';
        ...
        // Fallback to mail client if the serverless endpoint is not available.
        const subject = encodeURIComponent(`[EblogDev] Project enquiry from ${name} (${company})`);
        const body = encodeURIComponent(
            `Name: ${name}\nEmail: ${email}\nCompany: ${company}\n\nMessage:\n${message}`
        );
        window.location.href = `mailto:talktoelishaanthonylive@gmail.com?subject=${subject}&body=${body}`;
        fb.textContent = '✓ Opening your email client — fallback contact method activated.';
        fb.style.color = '#00e676';
        e.target.reset();
    }
});
